...
pub fn run(op: &Opts) -> Result<()> {
    ...
    let routes = external::gtfs::read::<Route>(&op.gtfs_dir.join("routes.txt"))?;
    info!("ℹ️ Add {} routes.", routes.len());
    gtfs.insert_routes(&routes)?;
    info!("  ✨ Success");

    let trips = external::gtfs::read::<Trip>(&op.gtfs_dir.join("trips.txt"))?;
    info!("ℹ️ Add {} trips.", trips.len());
    gtfs.insert_trips(&trips)?;
    info!("  ✨ Success");

    let stop_times = external::gtfs::read::<StopTime>(&op.gtfs_dir.join("stop_times.txt"))?;
